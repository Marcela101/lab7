var currentInitialPosition = new Vector(0, 0);
var varVerticalShift = new Vector(0, -0.5);
var varPlayerStop = false;
var varGlobalLevel = {};
...
function Level(plan) {
    ...
    currentInitialPosition = new Vector(0, 0);
    ...
    for (var y = 0; y < this.height; y++) {
        ...
        for (var x = 0; x < this.width; x++) {
            ...
            /* if (ch === '@') {
                 ...
                 currentInitialPosition = new Vector(x, y);
             }*/
            var Actor = actorChars[ch];
            if (Actor) {

                var newActor = new Actor(new Vector(x, y), ch);
...
Coin.prototype.act = function(step) {
    ...
    var wobblePosX = Math.sin(this.wobble) * wobbleDist;
    var wobblePosY = Math.cos(this.wobble) * wobbleDist;
...
Player.prototype.moveX = function(step, level, keys) {
    ...
    */
    ...
    //find if there is an obstacle in the newpos
    var obstacle = level.obstacleAt(newPos, this.size);

    //move if there is not an obstacle:
    if (obstacle == "lava") {
        //this.pos = currentInitialPosition.plus(varVerticalShift);
        restart();
    }
    if (obstacle != 'wall') {
        this.pos = newPos;
...
Player.prototype.moveY = function(step, level, keys) {
    ...
    */
    ...
    //The floor is also an obstacle:
    // if they are touching some obstacle:
    if (obstacle) {
...
async function restart() {
    console.log('You die...');
    varGlobalLevel.player.pos = currentInitialPosition.plus(varVerticalShift);
    varPlayerStop = true;
    Player.prototype.type = "playerDeath";
    await sleep(3000);
    Player.prototype.type = "player";
    varPlayerStop = false;
...
function runGame(plans, Display) {
    function startLevel(n) {
        ...
        varGlobalLevel = new Level(plans[n]);
        runLevel(varGlobalLevel, Display);
    }
    startLevel(0);
}